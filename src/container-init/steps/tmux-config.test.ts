...
describe('tmux-config step', () => {
  it('writes ~/.tmux.conf when missing', async () => {
    ...
    const conf = await c.fs.readFile('/home/vscode/.tmux.conf')
  ...
  it('does not overwrite an existing config', async () => {
    const c = await makeCtx()
    await c.fs.writeFile('/home/vscode/.tmux.conf', 'custom config')
    const r = await tmuxConfigStep.run(c)
    expect(r.ok).toBe(true)
    expect(await c.fs.readFile('/home/vscode/.tmux.conf')).toBe('custom config')
  })
})
